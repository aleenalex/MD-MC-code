...
int main (int argc, char *argv[])
{
    ...
    ind=8; start=0; N=108; rho=0.8442; set_tem=2;

box = pow ((double(N)/rho), 1./3.);
cout<<"ind  "<<ind<<"start   "<<start<<"N   "<<N<<"rho   "<<rho<<"box   "<<box<<"\n";
initialcondition();
simulation();
msdsample(5,0,5000); // or msdsample2()
rdfsample();
//velocity();
return 0;
}
...
void anderson(double nu, double dt)
{
    ...
    for(i=0;i<N;i++)
    {
        ...
        if(ran[i]<=nu*dt)
        {
            ...
            n1_ran=sqrt(double(-2)*log(u1_ran))*cos(a2);
            n2_ran=sqrt(double(-2)*log(u2_ran))*sin(a3);
            n3_ran=sqrt(double(-2)*log(u3_ran))*cos(a1);

            v[i].x=sqrt(T_req)*n1_ran;
            v[i].y=sqrt(T_req)*n2_ran;
            v[i].z=sqrt(T_req)*n3_ran;
            //cout<<"\n"<<v[i].x<<"\t"<<v[i].y<<"\t"<<v[i].z<<"\n";
...
void velocity()
{
    char IntStr[80];
    ofstream ofout;
    sprintf( IntStr, "velocity%d.dat",ind);
    ofout.open (IntStr, ofstream::out | ofstream::app);
    if (ofout.is_open())
    {
    for (int i=0;i<N;i++)
